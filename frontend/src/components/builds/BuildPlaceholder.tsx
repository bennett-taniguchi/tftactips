...
// type : loading or empty
// size: small is default if u pass large it will 40svh
// text: give singular noun: "build"
export default function BuildPlaceholder({ type, size, text }: BuildPlaceholderProps) {
  ...
  let message = type === "loading" ? "Fetching builds" : "No builds found";
  if(text) message = type === "loading" ? `Fetching ${text}` : `${text} Not found`;
if(size=="large")
  return (
    <div>
      <div
        className="rounded-4xl border border-white text-black mt-[-1svh] h-[20svh] w-[80svw] relative overflow-hidden transition-all duration-300 ease-in-out"
        style={{
