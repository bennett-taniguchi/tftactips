...
// Simple is expectation after parsing api data
type ChampionSimple = {
  name: string;
  cost: number;
  image: string;
...
type TFTraitRowProps ={
  name: string;
  description: string;
  levels: LevelSimple[];
  champions: ChampionSimple[];
  image: string
}
  const TFTTraitRow = ({name,description,levels,champions,image}: TFTraitRowProps) => {
  ...
  return (
    <div className="flex flex- md:flex-row bg-gray-800 rounded-lg p-4 my-2 text-white w-full">
      ...
      <div className="flex-grow mt-2 md:mt-0">
        <div className="mb-2">
          <h3 className="text-xl font-bold text-amber-300">{name}</h3>
