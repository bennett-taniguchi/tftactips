...
export const CrudService = {
  ...
  create: async (tableName: TableName, item: Item): Promise<void> => {
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new ApiError(error.error || 'Failed to create item', response.status);
      }
    } catch (error) {
      if (error instanceof ApiError) {
        throw error;
   ...
   */
  update: async (tableName: TableName, key: Key, updates: Item): Promise<void> => {
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new ApiError(error.error || 'Failed to update item', response.status);
      }
    } catch (error) {
      if (error instanceof ApiError) {
        throw error;
  ...
  delete: async (tableName: TableName, key: Key): Promise<void> => {
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new ApiError(error.error || 'Failed to delete item', response.status);
      }
    } catch (error) {
      if (error instanceof ApiError) {
        throw error;
